...
<div style="background:linear-gradient(135deg,#1e40af,#3b82f6);padding:1.5rem;border-radius:12px;color:white;margin-bottom:1.5rem;">
    <h2 style="font-weight:800;font-size:1.25rem;margin:0;">Daftar Ujian Kelas {{ auth()->user()->classRoom->name ?? '-' }}</h2>
...
<div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(300px, 1fr));gap:1.5rem;">
        ...
        <div class="card" id="card_schedule_{{ $s->id }}" style="border-top:4px solid {{ $isDone ? '#10b981' : ($s->is_active ? '#3b82f6' : '#9ca3af') }};">
            ...
            <div style="font-size:0.85rem;color:#4b5563;margin-bottom:1.5rem;line-height:1.5;">
                <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.25rem;">
                    <span>Durasi:</span> <strong>{{ $s->duration }} Menit</strong>
                </div>
                <div style="display:flex;align-items:center;gap:0.5rem;">
                    <span>KKM:</span> <strong>{{ $s->kkm }}</strong>
                </div>
            </div>
    ...
    @empty
        <div style="grid-column:1/-1;text-align:center;padding:3rem;background:#f9fafb;border-radius:12px;color:#6b7280;">
            ...
            <h3 style="font-weight:700;">Belum ada jadwal ujian</h3>
        ...
        schedules.forEach(scheduleId => {
            ...
            window.Echo.channel('exam.' + scheduleId)
                .listen('.ExamStateChanged', (e) => {
                    ...
                    if (!actionDiv || !badgeDiv || !cardDiv) return;

                    if (e.action === 'permission_changed') {
                        if ((e.payload?.user_id ?? null) !== currentUserId) return;

                        if (e.payload?.allowed) {
                            if (e.payload?.is_active) {
                            ...
                            cardDiv.style.borderTopColor = '#ef4444';
                            badgeDiv.innerHTML = '<span style="background:#fee2e2;color:#991b1b;padding:0.2rem 0.5rem;border-radius:6px;font-size:0.75rem;font-weight:700;">TIDAK DIIZINKAN</span>';
                            actionDiv.innerHTML = '<button class="btn" style="width:100%;background:#fee2e2;color:#991b1b;cursor:not-allowed;" disabled>Tidak Diizinkan</button>';
                        }
